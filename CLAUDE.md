...
- `download_official_papers.py` — downloads real JEE Main/NEET PDFs from
  ...
  claims without spot-checking actual records.
- `extract_pyq_images.py` — turns a downloaded Question Paper PDF (+
  optional matching Answer Key PDF) into real `ContentItem` rows. Confirmed
  by direct inspection: JEE Main question stems and MCQ options are
  embedded *images* in the PDF, not extractable text (anti-cheating
  randomization + math notation) — so `ContentItem` has `body_image`/
  `option_images` fields (parallel to `body`/`options`) storing the real
  official images directly, no OCR. `parse_question_paper()` walks each
  page's text+image blocks *in document order* (position, not `get_images()`
  list order, which isn't reliable reading order) to pair each question's
  stem image and 4 option images with its `Question Number`/`Question Id`
  metadata — see the module docstring for the exact block sequence this
  depends on. Numerical ("SA" type) questions have their answer embedded as
  plain text in the Question Paper itself (`Possible Answers : 42`) — no
  Answer Key needed for those. `correct_option()` in `quiz.py` handles
  image-based items via exact ID match (`option_images` set → `answer` is
  the correct option's raw ID) vs. the text-based prefix match otherwise.
  `import_questions()` groups by **exam session** as the `Topic` (e.g. "JEE
  Main 2026 Session 2 Shift 1") — the source PDFs give Subject + Question
  Number but no NCERT chapter, so session-based grouping is what the data
  actually supports; regroup into real chapters by hand later if wanted.
  Tests (`tests/test_extract_pyq_images.py`) use small **synthetic**
  fixture PDFs (`scraper/fixtures/pyq_*_sample.pdf`, generated by a
  throwaway script, made-up arithmetic questions) — never commit real
  downloaded exam PDFs as fixtures.
